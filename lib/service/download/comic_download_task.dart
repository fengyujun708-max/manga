...
import 'package:mangaverse/network/http/plugin/qjs_download_runtime.dart';
...
import 'package:mangaverse/page/comic_info/json/normal/normal_comic_all_info.dart'
    as normal;
import 'package:mangaverse/page/comic_info/method/get_plugin_detail.dart';
import 'package:mangaverse/page/download/adapters/download_chapter_adapter.dart';
import 'package:mangaverse/page/download/adapters/download_chapter_matcher.dart';
import 'package:mangaverse/page/download/models/download_chapter.dart';
import 'package:mangaverse/page/download/models/unified_comic_download.dart';
import 'package:mangaverse/service/download/download_cancel_signal.dart';
import 'package:mangaverse/service/download/download_progress_reporter.dart';
import 'package:mangaverse/service/download/models/download_task_json.dart';
import 'package:mangaverse/i18n/strings.g.dart';
import 'package:mangaverse/service/download/image_download.dart';
...
Future<void> unifiedDownloadTask(
  ...
  Future<void> ensureTaskRunning() async {
    ...
    final signaled = isDownloadCancelSignaled(task.comicId);
    if (signaled || currentTask == null || !currentTask.isDownloading) {
      logger.w(
        'ensureTaskRunning 取消任务: comicId=${task.comicId}, signaled=$signaled, currentTask=${currentTask != null}, isDownloading=${currentTask?.isDownloading}',
      );
...
Future<void> _saveUnifiedDownload({
  ...
  for (var index = 0; index < chapterResponses.length; index++) {
    ...
    logger.d(
      '_saveUnifiedDownload: index=$index, '
