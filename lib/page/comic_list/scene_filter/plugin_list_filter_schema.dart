...
class PluginListFilterSchema {
  const PluginListFilterSchema({required this.title, required this.fields});

  final String title;
  final List<PluginChoiceField> fields;

  factory PluginListFilterSchema.fromMap(Map<String, dynamic> map) {
    return PluginListFilterSchema(
  ...
  PluginResolvedListFilter resolve({
    ...
    for (final field in fields) {
      ...
      params.addAll(selected.flatResult);
      core.addAll(selected.coreResult);
      extern.addAll(selected.externResult);
    }

    return PluginResolvedListFilter(
...
class PluginChoiceOption {
  ...
  Map<String, dynamic> get coreResult => asJsonMap(result['core']);

  Map<String, dynamic> get externResult => asJsonMap(result['extern']);

  Map<String, dynamic> get flatResult {
    if (coreResult.isNotEmpty || externResult.isNotEmpty) {
      final next = Map<String, dynamic>.from(result);
      next.remove('core');
      next.remove('extern');
      return next;
    }
...
class PluginResolvedListFilter {
  const PluginResolvedListFilter({
