...
class DeleteReadChaptersUseCaseTest {
	...
	fun `history chapter not downloaded but in database deletes successfully`() = runTest {
		...
		val dbChapters = listOf(
			...
			com.mangaverse.app.core.db.entity.ChapterEntity(
				chapterId = it.id,
				mangaId = 100L,
	...
	fun `history chapter in database for remote downloaded manga deletes successfully`() = runTest {
		val remoteSource = mockk<com.mangaverse.app.parsers.model.ContentSource>()
		every { remoteSource.name } returns "RemoteSource"
		every { remoteSource.contentType } returns com.mangaverse.app.parsers.model.ContentType.MANGA
		...
		val localContent = com.mangaverse.app.local.domain.model.LocalContent(
			manga = manga,
