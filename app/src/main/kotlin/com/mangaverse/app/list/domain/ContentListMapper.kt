...
import com.mangaverse.app.list.ui.model.ContentDetailedListModel
...
import com.mangaverse.app.tracker.ui.feed.model.FeedItem
...
class ContentListMapper @Inject constructor(
	...
	suspend fun toListModelList(
		...
		manga.mapTo(destination) {
			toListModelImpl(
				...
				counters = counters,
				progress = progress,
				override = resolveDisplayOverride(manualOverrides[it.id]),
			)
		}
	}
	...
	suspend fun toRequestedListModelList(
		...
		requests.mapTo(destination) { request ->
			toListModelImpl(
				...
				counters = counters,
				progress = progress,
				override = resolveDisplayOverride(manualOverrides[request.manga.id]),
			)
		}
	}
	...
	suspend fun toListModel(
		...
		return toListModelImpl(
			...
			counters = null,
			progress = null,
			override = resolveDisplayOverride(dataRepository.getOverride(manga.id)),
		)
	}
	...
	suspend fun toFeedItem(logItem: TrackingLogItem) = FeedItem(
		...
		override = resolveDisplayOverride(dataRepository.getOverride(logItem.manga.id)),
	...
	suspend fun toFeedItems(logItems: List<TrackingLogItem>): List<FeedItem> {
		...
		return logItems.map { logItem ->
			FeedItem(
				...
				entityId = logItem.entityId,
				preferredLocalMangaId = logItem.preferredLocalMangaId,
				override = resolveDisplayOverride(manualOverrides[logItem.manga.id]),
				count = logItem.count ?: logItem.chapters.size,
				manga = logItem.manga,
				isNew = logItem.isNew,
	...
	private suspend fun toListModelImpl(
		...
		ListMode.LIST -> toCompactListModel(manga, options, pinnedIds, counters, progress, override)
		...
		ListMode.GRID,
		ListMode.COMPACT_GRID -> toGridModel(manga, options, pinnedIds, counters, progress, override)
	}

	private suspend fun getCounters(mangaIds: Collection<Long>): Map<Long, Int>? {
		return if (settings.isTrackerEnabled) {
			mangaUpdateRepository.getNewChaptersCounts(mangaIds)
		} else {
			null
		}
	}
	...
	private suspend fun getCounter(mangaId: Long, @Options options: Int, counters: Map<Long, Int>?): Int {
		return if (settings.isTrackerEnabled) {
			counters?.get(mangaId) ?: mangaUpdateRepository.getNewChaptersCount(mangaId)
		} else {
			0
		}
	}
