...
import com.mangaverse.app.parsers.ContentFavoriteFolder
...
import com.mangaverse.app.favourites.domain.FavoritesListQuickFilter
...
class FavouritesContainerViewModel @Inject constructor(
	...
	private fun buildActiveCategoryCounts(
		entries: List<com.mangaverse.app.favourites.data.FavouriteCategoryCountEntry>,
	...
	private suspend fun ensureCategory(title: String): FavouriteCategory {
		return favouritesRepository.findCategoryByTitle(title)
			?: favouritesRepository.createCategory(
				title = title,
				sortOrder = com.mangaverse.app.list.domain.ListSortOrder.NEWEST,
	...
	fun startFuzzyDuplicatesFix(tolerance: Int) {
		...
		duplicatesJob = launchJob(Dispatchers.Default) {
			try {
				...
				val fuzzyGroups = mutableListOf<List<com.mangaverse.app.parsers.model.Content>>()
				val similarityThreshold = tolerance / 100.0

				while (remaining.isNotEmpty()) {
					if (isDuplicatesCancellationRequested) break
					val root = remaining.removeAt(0)
					val duplicates = mutableListOf<com.mangaverse.app.parsers.model.Content>()
					duplicates.add(root)
	...
	private suspend fun performDeduplication(groupsToDelete: List<DuplicatesGroup>) {
		db.withTransaction {
			for (group in groupsToDelete) {
				...
				val repProjectionKey = com.mangaverse.app.core.model.ProjectionIdentityKeys.bindingKey(rep.url, rep.publicUrl)
				...
				for (dup in group.duplicates) {
					val projectionKey = com.mangaverse.app.core.model.ProjectionIdentityKeys.bindingKey(dup.url, dup.publicUrl)
...
private data class ProbedManga(
	val manga: com.mangaverse.app.parsers.model.Content,
	val isAlive: Boolean,
	val chapterCount: Int
)

data class DuplicatesGroup(
	val representative: com.mangaverse.app.parsers.model.Content,
	val duplicates: List<com.mangaverse.app.parsers.model.Content>,
	val isChecked: Boolean = true,
	val allOptions: List<com.mangaverse.app.parsers.model.Content> = emptyList()
)
