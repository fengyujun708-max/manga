...
import com.mangaverse.app.entitygraph.domain.titleBlockingKeys
import com.mangaverse.app.entitygraph.domain.titleSimilarityScore
...
class MergeFavoriteEntitiesUseCase @Inject constructor(
    ...
    suspend fun buildCandidateGroups(
        ...
        val higherPriorityIds = exactGroups.flatMapTo(HashSet()) { it.mangaIds }
        ...
        return (exactGroups + aliasGroups + fuzzyGroups).sortedWith(
            compareByDescending<MergeCandidateGroup> { it.id.contains(":alias:") }
            ...
            .also { groups ->
                Log.d(
                    TAG,
                    "buildCandidateGroups: total=${groups.size}, strategy=e:${exactGroups.size} a:${aliasGroups.size} f:${fuzzyGroups.size} fuzzy=${options.fuzzyEnabled}",
                )
    ...
    private suspend fun selectTargetEntityId(entityIds: List<Long>): Long {
        ...
        return entities.maxWithOrNull(
            compareBy<com.mangaverse.app.entitygraph.domain.Entity> { it.accessCount }
