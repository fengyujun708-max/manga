...
import com.mangaverse.app.core.extensions.DEFAULT_JAR_PRIORITY_ORDER_VALUE
import com.mangaverse.app.core.model.ZoomMode
import com.mangaverse.app.core.network.DoHProvider
import com.mangaverse.app.core.ui.compose.PanoramaAnimationSpeedMaxPercent
import com.mangaverse.app.core.ui.compose.PanoramaAnimationSpeedMinPercent
import com.mangaverse.app.explore.data.SourcesSortOrder
import com.mangaverse.app.list.domain.ListSortOrder
import com.mangaverse.app.core.prefs.VideoSuperResolutionMode
import com.mangaverse.app.core.prefs.VideoSuperResolutionShader
...
import com.mangaverse.app.parsers.util.find
...
import com.mangaverse.app.core.util.ext.getEnumValue
import com.mangaverse.app.core.util.ext.getSafeFloat
import com.mangaverse.app.core.util.ext.observeChanges
import com.mangaverse.app.core.util.ext.putAll
import com.mangaverse.app.core.util.ext.putEnumValue
import com.mangaverse.app.core.util.ext.takeIfReadable
import com.mangaverse.app.core.util.ext.toUriOrNull
import com.mangaverse.app.reader.domain.ReaderColorFilter
import java.io.File
...
class AppSettings @Inject constructor(@ApplicationContext private val context: Context) {
	...
	var mainNavItems: List<NavItem>
		get() {
			...
			if (rawStr != fixed.joinToString(",") { it.name }) {
				prefs.edit { putString(KEY_NAV_MAIN, fixed.joinToString(",") { it.name }) }
			}
			return fixed
		}
		set(value) {
	...
	// ===== MangaVerse 账号会话 =====
	var mangaVerseAuthToken: String
		get() = prefs.getString(KEY_MANGAVERSE_AUTH_TOKEN, "").orEmpty()
		set(value) = prefs.edit { putString(KEY_MANGAVERSE_AUTH_TOKEN, value) }

	var mangaVerseAuthUserId: String
		get() = prefs.getString(KEY_MANGAVERSE_AUTH_USER_ID, "").orEmpty()
		set(value) = prefs.edit { putString(KEY_MANGAVERSE_AUTH_USER_ID, value) }

	var mangaVerseAuthUsername: String
		get() = prefs.getString(KEY_MANGAVERSE_AUTH_USERNAME, "").orEmpty()
		set(value) = prefs.edit { putString(KEY_MANGAVERSE_AUTH_USERNAME, value) }

	val isMangaVerseLoggedIn: Boolean
		get() = mangaVerseAuthToken.isNotBlank() && mangaVerseAuthUserId.isNotBlank()

	fun clearMangaVerseSession() {
		prefs.edit {
			remove(KEY_MANGAVERSE_AUTH_TOKEN)
			remove(KEY_MANGAVERSE_AUTH_USER_ID)
			remove(KEY_MANGAVERSE_AUTH_USERNAME)
		}
	}

	// ===== 广告观看时长 =====
	var watchTimeFreeMinutesRemaining: Int
		get() = prefs.getSafeInt(KEY_WATCH_TIME_FREE_MINUTES, WATCH_TIME_FREE_DAILY)
		set(value) = prefs.edit { putInt(KEY_WATCH_TIME_FREE_MINUTES, value.coerceAtLeast(0)) }

	var watchTimeAdsWatchedToday: Int
		get() = prefs.getSafeInt(KEY_WATCH_TIME_ADS_TODAY, 0)
		set(value) = prefs.edit { putInt(KEY_WATCH_TIME_ADS_TODAY, value.coerceAtLeast(0)) }

	var watchTimeLastAdAt: Long
		get() = prefs.getLong(KEY_WATCH_TIME_LAST_AD_AT, 0L)
		set(value) = prefs.edit { putLong(KEY_WATCH_TIME_LAST_AD_AT, value) }

	var watchTimeDate: String
		get() = prefs.getString(KEY_WATCH_TIME_DATE, "").orEmpty()
		set(value) = prefs.edit { putString(KEY_WATCH_TIME_DATE, value) }

	var interstitialShownToday: Int
		get() = prefs.getSafeInt(KEY_INTERSTITIAL_SHOWN_TODAY, 0)
		set(value) = prefs.edit { putInt(KEY_INTERSTITIAL_SHOWN_TODAY, value.coerceAtLeast(0)) }

	/** 是否已看完今日全部广告（当天免费无限观看） */
	val isWatchTimeUnlimitedToday: Boolean
		get() = watchTimeAdsWatchedToday >= WATCH_TIME_MAX_ADS_PER_DAY

	/** 跨天重置观看时长状态 */
	fun rollWatchTimeIfNeeded(today: String = java.time.LocalDate.now().toString()) {
		if (watchTimeDate != today) {
			prefs.edit {
				putString(KEY_WATCH_TIME_DATE, today)
				putInt(KEY_WATCH_TIME_FREE_MINUTES, WATCH_TIME_FREE_DAILY)
				putInt(KEY_WATCH_TIME_ADS_TODAY, 0)
				putLong(KEY_WATCH_TIME_LAST_AD_AT, 0L)
				putInt(KEY_INTERSTITIAL_SHOWN_TODAY, 0)
			}
		}
	}
	...
	val readerTranslationPipelineMode: com.mangaverse.app.core.prefs.ReaderTranslationPipelineMode
		get() = com.mangaverse.app.core.prefs.ReaderTranslationPipelineMode.TWO_STAGE
	...
	companion object {
		...
		// ===== 广告观看时长 =====
		const val WATCH_TIME_FREE_DAILY = 30 // 每日免费 30 分钟
		const val WATCH_TIME_AD_REWARD = 30 // 看一次广告 +30 分钟
		const val WATCH_TIME_MAX_ADS_PER_DAY = 2 // 每日最多 2 次广告（看完当天免费）
		const val WATCH_TIME_AD_COOLDOWN_MS = 10 * 60 * 1000L // 广告间隔 10 分钟
		...
		// MangaVerse 账号会话
		const val KEY_MANGAVERSE_AUTH_TOKEN = "mangaverse_auth_token"
		const val KEY_MANGAVERSE_AUTH_USER_ID = "mangaverse_auth_user_id"
		const val KEY_MANGAVERSE_AUTH_USERNAME = "mangaverse_auth_username"

		// MangaVerse 商业化（能量 / 会员 / 任务）
		// 广告观看时长
		const val KEY_WATCH_TIME_FREE_MINUTES = "mangaverse_watch_free_minutes"
		const val KEY_WATCH_TIME_ADS_TODAY = "mangaverse_watch_ads_today"
		const val KEY_WATCH_TIME_LAST_AD_AT = "mangaverse_watch_last_ad_at"
		const val KEY_WATCH_TIME_DATE = "mangaverse_watch_date"
		const val KEY_INTERSTITIAL_SHOWN_TODAY = "mangaverse_interstitial_today"
	...
	var filterPillDefaultType: com.mangaverse.app.parsers.model.ContentType
		get() {
			val name = prefs.getString(KEY_FILTER_PILL_DEFAULT, com.mangaverse.app.parsers.model.ContentType.MANGA.name) ?: com.mangaverse.app.parsers.model.ContentType.MANGA.name
			return try { enumValueOf(name) } catch (e: Exception) { com.mangaverse.app.parsers.model.ContentType.MANGA }
		}
		set(value) = prefs.edit { putString(KEY_FILTER_PILL_DEFAULT, value.name) }

	var filterPillSwipeLeftType: com.mangaverse.app.parsers.model.ContentType
		get() {
			val name = prefs.getString(KEY_FILTER_PILL_LEFT, com.mangaverse.app.parsers.model.ContentType.VIDEO.name) ?: com.mangaverse.app.parsers.model.ContentType.VIDEO.name
			return try { enumValueOf(name) } catch (e: Exception) { com.mangaverse.app.parsers.model.ContentType.VIDEO }
		}
		set(value) = prefs.edit { putString(KEY_FILTER_PILL_LEFT, value.name) }

	var filterPillSwipeRightType: com.mangaverse.app.parsers.model.ContentType
		get() {
			val name = prefs.getString(KEY_FILTER_PILL_RIGHT, com.mangaverse.app.parsers.model.ContentType.NOVEL.name) ?: com.mangaverse.app.parsers.model.ContentType.NOVEL.name
			return try { enumValueOf(name) } catch (e: Exception) { com.mangaverse.app.parsers.model.ContentType.NOVEL }
		}
