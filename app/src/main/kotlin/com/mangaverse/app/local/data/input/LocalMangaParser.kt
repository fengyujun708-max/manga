package com.mangaverse.app.local.data.input
...
import com.mangaverse.app.core.util.AlphanumComparator
import com.mangaverse.app.core.util.MimeTypes
import com.mangaverse.app.core.util.ext.URI_SCHEME_ZIP
import com.mangaverse.app.core.util.ext.isDirectory
import com.mangaverse.app.core.util.ext.isFileUri
import com.mangaverse.app.core.util.ext.isImage
import com.mangaverse.app.core.util.ext.isRegularFile
...
import com.mangaverse.app.core.util.ext.toListSorted
import com.mangaverse.app.local.data.ContentIndex
import com.mangaverse.app.local.data.hasZipExtension
import com.mangaverse.app.local.data.isZipArchive
import com.mangaverse.app.local.data.output.LocalContentOutput.Companion.ENTRY_NAME_INDEX
	...
	suspend fun getContent(withDetails: Boolean): LocalContent {
		return kotlinx.coroutines.runInterruptible(kotlinx.coroutines.Dispatchers.IO) {
			resolveFsAndPath().use { (fileSystem, rootPath) ->
				val index = com.mangaverse.app.local.data.ContentIndex.read(fileSystem, rootPath / com.mangaverse.app.local.data.output.LocalContentOutput.ENTRY_NAME_INDEX)
				...
				if (mangaInfo != null) {
					...
					val resolvedLocalSource = when (mangaInfo.source?.contentType) {
						else -> com.mangaverse.app.core.model.LocalMangaSource
					}
				...
				var inferedSource: com.mangaverse.app.parsers.model.ContentSource = com.mangaverse.app.core.model.LocalMangaSource
				...
				if (withDetails || shouldGenerateIndex) {
					var detectedSource: com.mangaverse.app.parsers.model.ContentSource = com.mangaverse.app.core.model.LocalMangaSource
					val chapters = fileSystem.listRecursively(rootPath)
						.mapNotNullTo(HashSet()) { path ->
							when {
								...
								com.mangaverse.app.local.data.hasZipExtension(path.name) -> path
								else -> null
							}
						}.sortedWith(compareBy(com.mangaverse.app.core.util.AlphanumComparator()) { x -> x.toString() })
					detectedChapters = chapters.mapIndexed { i, p ->
						...
						val chapter = com.mangaverse.app.parsers.model.ContentChapter(
							id = "$i$s".longHashCode(),
				...
				val content = com.mangaverse.app.parsers.model.Content(
				...
				if (shouldGenerateIndex && detectedChapters != null) {
					runCatchingCancellable {
						val newIndex = com.mangaverse.app.local.data.ContentIndex(null)
						...
						java.io.File(rootFile, com.mangaverse.app.local.data.output.LocalContentOutput.ENTRY_NAME_INDEX).writeText(newIndex.toString())
					}.onFailure {
				...
				content
			}.let { com.mangaverse.app.local.domain.model.LocalContent(it, rootFile) }
			}
		}
	}
