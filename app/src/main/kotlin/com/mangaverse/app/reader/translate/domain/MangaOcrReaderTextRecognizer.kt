...
/**
 * MangaOcr 识别器 —— 改为调用后端 /api/ocr。
 * 保留 setDiagnosticsEmitter/consumeLastDiagnostics 接口（空实现）。
 */
class MangaOcrReaderTextRecognizer @Inject constructor(
	@ApplicationContext private val context: Context,
) : ReaderTextRecognizer {

	private val LOG_TAG = "MangaOcrReaderTextRecognizer"
	private var diagnosticsEmitter: (() -> String) -> Unit = {}
	...
	data class Diagnostics(val message: String = "")
	...
	private suspend fun recognizeServer(bitmap: Bitmap): List<OcrTextBlock> = withContext(Dispatchers.IO) {
		try {
			...
			lastDiagnostics = Diagnostics("error: ${e.message}")
			emptyList()
		}
	}
}
