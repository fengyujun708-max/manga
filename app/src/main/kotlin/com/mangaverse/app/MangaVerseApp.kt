...
open class MangaVerseApp : BaseApp() {
	...
	private fun configureLeakCanary() {
		...
		// LeakCanary 仅存在于 debug/nightly 构建，通过反射调用避免 release 编译依赖
		try {
			val configClass = Class.forName("leakcanary.LeakCanary")
			val configField = configClass.getField("config")
			val current = configField.get(null)
			val copyMethod = current.javaClass.getMethod("copy")
			val builder = copyMethod.invoke(current)
			val dumpHeapField = builder.javaClass.getField("dumpHeap")
			dumpHeapField.setBoolean(builder, getDebugPreferences(this).getBoolean(KEY_LEAK_CANARY, false) && !isOplusDevice)
			val buildMethod = builder.javaClass.getMethod("build")
			configField.set(null, buildMethod.invoke(builder))
		} catch (_: Throwable) {
			// LeakCanary 不在 classpath（release 构建），忽略
		}
	}
	...
	private companion object {
		...
		private const val TAG = "MangaVerseFatal"
