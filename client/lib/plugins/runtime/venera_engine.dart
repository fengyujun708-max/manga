...
/// 加密：通过 sendMessage 桥接 Dart crypto
class VeneraEngine {
  ...
  Future<void> init() async {
    ...
    // 加密桥接（Dart crypto 实现，同步返回值）
    _runtime!.setupBridge('crypto', (dynamic args) {
      final m = args as Map;
      final op = m['op']?.toString() ?? '';
      final data = m['data']?.toString() ?? '';
      switch (op) {
        case 'md5': return md5.convert(utf8.encode(data)).toString();
        case 'sha1': return sha1.convert(utf8.encode(data)).toString();
        case 'sha256': return sha256.convert(utf8.encode(data)).toString();
        case 'sha512': return sha512.convert(utf8.encode(data)).toString();
        case 'hmac':
          final key = m['key']?.toString() ?? '';
          final msg = m['msg']?.toString() ?? '';
          final algo = m['algo']?.toString() ?? 'sha256';
          return Hmac(_hashAlgo(algo), utf8.encode(key)).convert(utf8.encode(msg)).toString();
        case 'b64encode': return base64.encode(utf8.encode(data));
        case 'b64decode':
          try {
  ...
  Future<bool> executeSource(String sourceId, String jsCode) async {
    ...
    ''');
    // 通过运行时工厂执行（含实例化 + init()）
    final esc = jsCode.replaceAll('\\', '\\\\').replaceAll("'", "\\'");
    _runtime!.evaluate(
        'globalThis.__executeSource__(\'$esc\', \'$sourceId\');');
    // 泵送 Promise 直到源注册
    for (var i = 0; i < 300; i++) {
  ...
  /// 求值 JS 并等待 Promise 完成（返回 Dart 值）
...
''';
  ...
  /// 内部：执行 pending job 循环
  Future<dynamic> pumpAndGet(String jsExpr) async {
    for (var i = 0; i < 600; i++) {
      _runtime!.executePendingJob();
      final r = _runtime!.evaluate('globalThis.$jsExpr');
      final val = r.rawResult;
