...
插件列表：<https://github.com/deretame/MangaVerse-plugin-list>
...
- 前往 [Releases 页面](https://github.com/deretame/MangaVerse/releases) 下载 `app-arm64-v8a-release.apk`。
...
安装完成后，你可以通过应用菜单找到 **MangaVerse**，或者在终端输入：
...
macOS 用户推荐使用 Homebrew 来安装和管理 MangaVerse。当然，你也可以直接下载 DMG 文件手动安装。
...
**3. 更新 MangaVerse**
当 MangaVerse 发布新版本后，你可以通过以下命令一键升级：
...
**4. 彻底卸载**
如果你需要卸载 MangaVerse，并希望同时清理底层的数据库、缓存和配置文件，请务必加上 `--zap` 参数（**注意：这不会删除你手动导出的文件，但会清空 App 内部数据**）：
...
1. 前往 [Releases 页面](https://github.com/deretame/MangaVerse/releases) 下载最新的 `MangaVerse-macOS.dmg`。
2. 双击打开 `.dmg` 文件，将 `MangaVerse.app` 拖入 `Applications`（应用程序）文件夹。

> ⚠️ **首次启动注意事项 (macOS Gatekeeper)**
> 由于本项目为开源免费软件，未进行 Apple 开发者签名。首次打开时，如果系统提示 **“应用已损坏”** 或 **“无法验证开发者”**，请在终端中执行以下命令予以放行，之后即可正常使用：
>
> ```bash
> xattr -cr /Applications/MangaVerse.app
> ```
>
> _(备选方法：在“应用程序”文件夹中找到 MangaVerse，按住 `Control` 键点击应用图标，然后在弹出的菜单中选择“打开”。)_

### 📱 iOS / iPadOS

由于 iOS 系统的封闭性，未上架 App Store 的应用需要通过“侧载 (Sideloading)”并自行签名才能安装。请前往 [Releases 页面](https://github.com/deretame/MangaVerse/releases) 下载无签名的 `MangaVerse-iOS.ipa` 文件，并使用以下工具之一进行安装：
...
- 项目仓库：**[MangaVerse](https://github.com/deretame/MangaVerse)**
- 联系方式：**[telegram(电报)](https://t.me/breeze_zh_cn)**

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://star-history.dera.page/svg?repos=deretame/MangaVerse&type=Date&theme=dark" />
    <source media="(prefers-color-scheme: light)" srcset="https://star-history.dera.page/svg?repos=deretame/MangaVerse&type=Date" />
